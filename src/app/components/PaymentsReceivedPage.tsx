...
import { Search, Plus, Eye, ArrowDownCircle, CreditCard, Building2 } from 'lucide-react';
import { mockPaymentsReceived } from '@/app/data/mockData';
...
export function PaymentsReceivedPage() {
  ...
  const payments = mockPaymentsReceived;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Payments Received</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Track payments received from customers</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex items-center gap-2 mb-1"><ArrowDownCircle className="w-4 h-4 text-green-500" /><p className="text-xs text-slate-500 font-semibold">Total Received</p></div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  <td className="py-3 px-4 text-sm font-medium text-teal-600 dark:text-teal-400">{p.paymentNumber}</td>
                  <td className="py-3 px-4 text-sm text-slate-700 dark:text-slate-300">{p.customerName}</td>
                  <td className="py-3 px-4 text-sm text-slate-500">{p.invoiceId}</td>
                  ...
                  <td className="py-3 px-4 text-right text-sm font-semibold text-green-600">${p.amount.toFixed(2)}</td>
                </tr>
              ))}
